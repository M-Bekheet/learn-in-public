...
public class Calculator {
  ...
  public void updateChart() {
    ...
    double lastSaving = savings.doubleValue() * 12 * (1 + rate);
    // interest

    savingsLine.getData().add(new XYChart.Data<Number, Number>(0, lastSaving));
    interestLine.getData().add(new XYChart.Data<Number, Number>(0, fixedSavings * rate));

    for (int i = 1; i <= 30; i++) {
      double oldSaving = lastSaving;
